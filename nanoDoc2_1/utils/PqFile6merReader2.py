...
class PqReader:
    ...
    def __init__(self, path,ref,minreadlen,chrom,strand,start=0, end=0, maxreads = 500,IndelStrict = False):
        ...
        self.maxreads = int(maxreads * 1.4)# take little more sample since some reads disqualify
        ...
        self.load(chrom, start, end, strand)
    ...
    def load(self,chr, start,end, strand):
        ...
        if self.firstloadpos == 0:
            self.firstloadpos = start
        self.lastloadpos = end
    ...
    def getRowData(self, chr, strand, pos,takecnt=-1):
        ...
        if pos-self.start == 100: # reload at position 100
            self.load(chr, pos, self.end, strand)
