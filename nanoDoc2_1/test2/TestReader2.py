from nanoDoc2_1.test2.PqReader import TraceSeqReader
from matplotlib import pyplot as plt
from nanoDoc2.graph.GraphManager import GraphManager
from numba import jit

def plotGraph(traces,infos,pathout):
...
if __name__ == "__main__":
    ...
    # load or build index
    rows,infos = fr.getRowData(1119,takecnt=maxreads)
    print("lenrow",len(rows))
    print("leninfo",len(infos))
    pathout = "/data/nanopore/nanoTune/test/testr1.pdf"
    plotGraph(rows,infos,pathout)

    traces,infos = fr.getRowData(1252,takecnt=maxreads)
    # print(len(traces))
    # print(traces)
    pathout = "/data/nanopore/nanoTune/test/testr2.pdf"
    plotGraph(rows,infos,pathout)
