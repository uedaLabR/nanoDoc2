...
def flipplopViterbiEach(lgenome,chrom,strand,r_st,r_en,q_st,q_en,trace,move):
   ...
   #possiblemove = move
    ...
    return seq, cigar, left, traceboundary, frombasecaller_idx,possiblemove_idx


import signalalign.OutputUtils as ou
from numba.typed import List
def viterbi(lgenome,compactTrace,trace,compactTracePositionMap,strand,q_st,q_en,possiblemove_idx,frombasecaller_idx):
    ...
    return seq,cigar,left,traceboundary
...
def addPossibleChangePoint(trace, move):
    ...
    tt = trace.T
    for flipflop in tt:
        algo_c = rpt.KernelCPD(kernel="linear", min_size=3).fit(flipflop)
...
def viterbiEach(compactTrace, localgenome, possiblemove_idx,frombasecaller_idx,ctstart,m_range):
    ...
    maxscore = 0
    penalty = 0.1
    untransPenaltyFordefultsegment = 0.6
    traceRatios = None
        ...
        for m in range(intervallen):
            ...
            if inrange:
                if m == 0:
                    ...
                    if possiblemove_idx[m] in frombasecaller_idx:
                        unTransScore = unTransScore * untransPenaltyFordefultsegment
                    ...
                    if n > MAXDEL_SIZE:
                        ...
                        for dlen in range(2, MAXDEL_SIZE + 1):
                            ...
                            deltransScore = scorematrix[n - dlen][m - 1] + stateP - IndelPenalty - (dlen * IndexExtentionpenalty)
                            if deltransScore > maxdscore:
                                maxdscore = deltransScore
                                maxdin = dlen

                    if transScore > unTransScore and transScore > maxdscore:
                        scorematrix[n][m] = transScore + wscore
                        movematrix[n][m] = DIOGONAL_MOVE
                    elif unTransScore > maxdscore:
                        scorematrix[n][m] = unTransScore + wscore
                        movematrix[n][m] = HORIZONTAL_MOVE
                    else:
                        scorematrix[n][m] = maxdscore + wscore
