...
def binned(trimtrace, traceItv,trimUnitLength,rseq,signal):
    ...
    if eidx <1:
        return None
    middlelen = index[eidx-1] -index[idx]
    if middlelen > DATA_LENGTH_Trace:
        return None

    return binsignal,bintrace
...
class PqReader:
    ...
    def getRowData(self, chr, strand, pos,takecnt=-1):
        ...
        signals,traces,sampledlen = self.getFormattedData(strand, pos,rseq,takecnt)
        ...
        return signals,traces,sampledlen
    ...
    def getOneRow(self,row,strand, pos,rseq):
        ...
        signal = signal[tracestart*10:traceend*10]
        #print(signal)
        #
        ret = binned(trace,traceItv,DATA_LENGTH_UNIT,rseq,signal)
        if ret == None:
            return None

        binsignal,bintrace = ret
        return binsignal,bintrace
