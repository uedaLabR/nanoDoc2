...
class PqReader:
    ...
    def __init__(self, path,ref,minreadlen,strand,start=0, end=0, maxreads = 500,IndelStrict = False):
        ...
        #loading data
        print(chr)
        self.load(chr, start, end, strand)
    ...
    def randomsample(self, pos, data, ntake, indexes):
        ...
        if indexes is None:
            ...
            # datainpos['length'] = datainpos['end'] - datainpos['start']
            # datainpos.sort_values('length', ascending=False)
    ...
    def load(self,chr, start,end,strand):
        ...
        strand = True
        query = 'start <= ' + str(start+100) + ' & end >= ' + str(end-100) + \
                ' & chr == "' + str(chr) + '" & strand == ' + str(strand) + '' + \
                ' & (end-start) >= ' + str(self.minreadlen)
